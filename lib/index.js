            ...
            if (listResult['DIDL-Lite']) {
              ...
              if (content.container) {
                ...
                for (let i = 0; i < content.container.length; i++) {
                  ...
                  // customFields is an object (map) of outputFieldName to a DLNA/UPnP field
                  // For example:
                  //   {
                  //      'albumArt': 'upnp:albumArtURI'
                  //   }
                  // This could be done more generally with caller-supplied functions, but I
                  // found returning useful error messages in that case was very difficult.
                  if (options.customFields) {
                    for (const [jsfield, xmlfield] of Object.entries(options.customFields)) {
                      if (xmlfield in content.container[i]) {
                        browseResult.container[i][jsfield] = content.container[i][xmlfield][0];
                      }
                      else if (options.debugCustomFieldNames) {
                        console.log(`Unable to find "${xmlfield}" in container result for custom field "${jsfield}"`);
                      }
                    }
                  }
                }
              ...
              if (content.item) {
                ...
                for (let i = 0; i < content.item.length; i++) {
                  ...
                  if (options.customFields) {
                    for (const [jsfield, xmlfield] of Object.entries(options.customFields)) {
                      if (xmlfield in content.item[i]) {
                        browseResult.item[i][jsfield] = content.item[i][xmlfield][0];
                      }
                      else if (options.debugCustomFieldNames) {
                        console.log(`Unable to find "${xmlfield}" in item result for custom field "${jsfield}"`);
                      }
                    }
                  }
                }
